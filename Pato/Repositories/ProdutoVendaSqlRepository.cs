    ...
    public class ProdutoVendaSqlRepository : DBContext, IProdutoVendaRepository
    {
        public void Create(ProdutoVenda produtoVenda)
        {
            try
            {
                ...
                cmd.CommandText = @"INSERT INTO ProdutoVenda 
                    VALUES (@idVenda, @idProduto, @qtd)";
        ...
        public void Update(int id, ProdutoVenda produtoVenda)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = connection;
                cmd.CommandText = @"UPDATE ProdutoVenda 
                    SET vendaId = @idVenda, produtoId = @idProduto, qtd = @qtd
                    WHERE IdPv = @id";
        ...
        public List<ProdutoVenda> ReadByVenda(int id)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = connection;
                cmd.CommandText = @"SELECT ProdutoVenda.*, Produto.Nome as Produto FROM ProdutoVenda JOIN Produto ON ProdutoVenda.produtoId = Produto.IdProduto WHERE vendaId = @id";
